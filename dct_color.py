...
def compress_image(image):
    ...
    compressed_image = np.zeros((height, width, channels))

    for c in range(channels):
        for i in range(0, height, 8):
            for j in range(0, width, 8):
                block = get_image_block(image[:, :, c], i, j)
                dct_block = dct2D(block)
                compressed_image[i:i+8, j:j+8, c] = dct_block
...
def decompress_image(compressed_image):
    ...
    decompressed_image = np.zeros((height, width, channels))

    for c in range(channels):
        for i in range(0, height, 8):
            for j in range(0, width, 8):
                block = get_image_block(compressed_image[:, :, c], i, j)
...
original_image = cv2.imread('og.jpg')  # Đọc ảnh màu
...
if original_image is None:
    ...
    print(f'Kích thước ảnh: {width}x{height}x{channels}')
